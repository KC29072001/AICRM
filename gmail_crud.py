...
# # updated Delete and Read with gmail links 
# from fastapi import FastAPI, HTTPException
# from pydantic import BaseModel
# import os
# from dotenv import load_dotenv
...
#                     logger.warning(f"Token for {gmail_address} lacks required scopes. Expected: {scopes}, Found: {creds.scopes}")
#                     os.remove(token_file)  # Delete the token file to force re-authentication
#                     creds = None
#                 else:
#                     CREDENTIALS_STORAGE[gmail_address] = creds
#                     logger.info(f"Loaded credentials from {token_file} for {gmail_address}")
...
#                 logger.info(f"Refreshing credentials for {gmail_address}")
#                 creds.refresh(GoogleRequest())
#                 # After refresh, ensure the scopes are still correct
#                 if not all(scope in creds.scopes for scope in scopes):
#                     logger.warning(f"Refreshed token for {gmail_address} lacks required scopes. Expected: {scopes}, Found: {creds.scopes}")
#                     creds = None
#                 else:
#                     CREDENTIALS_STORAGE[gmail_address] = creds
#                     safe_email = gmail_address.replace('@', '_').replace('.', '_')
...
#         raise HTTPException(
#             status_code=401,
#             detail=f"No valid credentials for {gmail_address}. Please authenticate via /auth/start to grant the required scope: https://www.googleapis.com/auth/gmail.modify."
#         )
...
#             is_draft = 'DRAFT' in message_data.get('labelIds', [])
#             thread_id = message_data.get('threadId')
            
#             # Create new MIME message
#             # Append a note for sent messages to indicate replacement
#             body_content = request.data['body']
#             if not is_draft:
#                 body_content += "\n\n(This message replaces a previous email in this thread.)"
...
#                 if not draft_id:
#                     raise HTTPException(status_code=404, detail=f"Draft with message ID {message_id} not found")
...
#                 sent_message = service.users().drafts().send(userId='me', body={'id': updated_draft['id']}).execute()
#                 logger.info(f"Updated and sent draft with ID {sent_message['id']} for {request.gmail_address}")
                
#                 return {
#                     "status": "success",
#                     "operation": request.operation,
#                     "result": sent_message,
#                     "message": f"Draft updated and sent with ID {sent_message['id']}"
#                 }
#             else:
#                 # Create a new draft in the same thread
#                 draft = service.users().drafts().create(
#                     userId='me',
...
#                 # Trash the original message (default to true)
#                 if request.data.get('delete_original', True):
#                     try:
#                         service.users().messages().trash(userId='me', id=message_id).execute()
#                         logger.info(f"Moved original message with ID {message_id} to Trash for {request.gmail_address}")
#                     except HttpError as e:
#                         logger.warning(f"Failed to move original message {message_id} to Trash: {str(e)}")
                
#                 logger.info(f"Created and sent new message with ID {sent_message['id']} in thread {thread_id} for {request.gmail_address}")
#                 return {
#                     "status": "success",
#                     "operation": request.operation,
#                     "result": sent_message,
#                     "message": f"Original message moved to Trash; new message sent in thread {thread_id} with ID {sent_message['id']}"
#                 }
...
#             logger.info(f"Moved messages with IDs: {message_ids} to Trash for {request.gmail_address}")
#             return {
#                 "status": "success",
#                 "operation": request.operation,
#                 "result": [],
#                 "message": f"Messages with IDs {message_ids} moved to Trash"
#             }
...
# UPDATE Operation for appending messages to given [ message id, email, Subject ] it appends Body message 
from fastapi import FastAPI, HTTPException
from pydantic import BaseModel
import os
from dotenv import load_dotenv
...
async def manage_leads(request: LeadRequest):
    try:
        ...
        if request.operation == OperationType.CREATE:
            ...
            # Fetch the message to verify it exists and get thread information
            try:
                message_data = service.users().messages().get(userId='me', id=message_id).execute()
            except HttpError as e:
                logger.error(f"Failed to fetch message with ID {message_id}: {str(e)}")
            ...
            if is_draft:
                ...
                if not draft_id:
                    logger.error(f"Draft with message ID {message_id} not found in draft list")
                    raise HTTPException(status_code=404, detail=f"Draft with message ID {message_id} not found")
                ...
                try:
                    ...
                    logger.error(f"Failed to update draft with ID {draft_id}: {str(e)}")
                ...
                try:
                    ...
                    logger.error(f"Failed to send updated draft with ID {updated_draft['id']}: {str(e)}")
                    raise HTTPException(status_code=500, detail=f"Failed to send updated draft: {str(e)}")
                ...
                # For sent messages, create a new message in the same thread
                # Fetch the original message's headers to get Message-ID for threading
                ...
                if not original_message_id:
                    logger.warning(f"Could not find Message-ID for original message {message_id}")
                
                # Create new MIME message with threading headers
                body_content = request.data['body']
                message = MIMEText(body_content)
                message['to'] = request.data['to']
                # Use the original subject if the new subject hasn't changed significantly
                new_subject = request.data['subject']
                if original_subject and new_subject != original_subject:
                    # Prepend "Re:" if not already present to help Gmail thread
                    if not new_subject.lower().startswith('re:'):
                        new_subject = f"Re: {new_subject}"
                message['subject'] = new_subject if original_subject else request.data['subject']
                ...
                try:
                    ...
                    logger.error(f"Failed to create new draft in thread {thread_id}: {str(e)}")
                    raise HTTPException(status_code=500, detail=f"Failed to create new draft: {str(e)}")
                
                # Send the new draft
                try:
                    sent_message = service.users().drafts().send(userId='me', body={'id': draft['id']}).execute()
                except HttpError as e:
                    logger.error(f"Failed to send new draft with ID {draft['id']}: {str(e)}")
                    raise HTTPException(status_code=500, detail=f"Failed to send new draft: {str(e)}")
                
                # Verify the new message is in the correct thread
                if sent_message.get('threadId') != thread_id:
                    logger.warning(f"New message thread ID {sent_message.get('threadId')} does not match original thread ID {thread_id}")
                ...
                return {
                    ...
                    "operation": request.operation,
                    "result": sent_message,
                    "message": f"Sent messages cannot be edited. A new message was sent in thread {thread_id} with ID {sent_message['id']}."
                }
