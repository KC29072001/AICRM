...
# # common point for info-ret-agent and odoo-agent 
# import os
# from dotenv import load_dotenv
...
# # Import from odoo-agent.py
# from odoo_agent import (
#     OdooAuthRequest,
#     ChatRequest,
#     authenticate_odoo,
#     chat,
...
# # Import from info-retrieval-agent.py
# from info_retrieval_agent import (
#     LocateRequest,
#     auth_start,
#     auth_callback,
#     auth_success,
...
# app = FastAPI(title="AICRM Combined API")
...
# # Create routers for each agent
# odoo_router = APIRouter(prefix="/odoo", tags=["Odoo Agent"])
# info_router = APIRouter(prefix="/info", tags=["Info Retrieval Agent"])

# # Include Odoo agent endpoints
# @odoo_router.post("/api/v1/chat")
# async def odoo_chat(request: ChatRequest):
#     return await chat(request)

# # Include Info Retrieval agent endpoints
# @info_router.post("/api/v1/retrieve")
# async def info_retrieve(request: LocateRequest):
#     return await retrieve_info(request)

# # Google auth endpoints
# @app.get("/auth/start")
# async def info_auth_start(gmail_address: str, request: Request):
#     return await auth_start(gmail_address, request)

# @app.get("/auth/callback")
# async def info_auth_callback(request: Request):
#     return await auth_callback(request)

# @app.get("/auth/success")
# async def info_auth_success():
#     return await auth_success()

# # Shared /auth/odoo endpoint
# @app.post("/auth/odoo")
# async def auth_odoo(request: OdooAuthRequest):
#     try:
#         common = xmlrpc.client.ServerProxy(f"{request.odoo_url}/xmlrpc/2/common")
...
#         logger.info(f"Successfully authenticated Odoo for {request.odoo_username}")
#         return {
#             "message": f"Odoo authentication successful. You can now use the /info/api/v1/retrieve endpoint with source='odoo' and gmail_address='{request.odoo_username}'."
#         }
#     except Exception as e:
#         logger.error(f"Odoo authentication failed: {str(e)}")
#         raise HTTPException(status_code=500, detail=f"Odoo authentication failed: {str(e)}")
...
# common point for info-ret-agent, odoo-agent, and gmail-agent
import os
from dotenv import load_dotenv
from fastapi import FastAPI, HTTPException, APIRouter, Request
from fastapi.middleware.cors import CORSMiddleware
...
from odoo_agent import (
    OdooAuthRequest,
    ChatRequest as OdooChatRequest,
    authenticate_odoo,
    chat as odoo_chat,
)
...
# Import from gmail-agent.py
from gmail_agent import (
    ChatRequest as GmailChatRequest,
    process_query as gmail_process_query,
)
...
# Include Odoo agent endpoints
@odoo_router.post("/api/v1/chat")
async def odoo_chat_endpoint(request: OdooChatRequest):
    return await odoo_chat(request)
...
# Include Gmail agent endpoint
@gmail_router.post("/api/v1/chat")
async def gmail_chat(request: GmailChatRequest):
    ...
    logger.info(f"Received Gmail query: {request.query} for Gmail address: {request.gmail_address}")
    response = await gmail_process_query(request.query, request.gmail_address)
    return response

# Google auth endpoints (from info-retrieval-agent)
@app.get("/auth/start")
async def info_auth_start(gmail_address: str, request: Request):
    return await auth_start(gmail_address, request)
...
async def auth_odoo(request: OdooAuthRequest):
    try:
        ...
        logger.info(f"Successfully authenticated Odoo for {request.odoo_username}")
        return {
            "message": f"Odoo authentication successful. You can now use the /info/api/v1/retrieve endpoint with source='odoo' and gmail_address='{request.odoo_username}' and can also use /odoo/api/v1/chat endpoint with username='{request.odoo_username}'."
        }
    except Exception as e:
        logger.error(f"Odoo authentication failed: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Odoo authentication failed: {str(e)}")
