...
# normal Retrieval of Activity log along with messages + Time conversion for each log to IST
# auth reset 
from fastapi import FastAPI, HTTPException, Request as FastAPIRequest
from fastapi.responses import RedirectResponse
from pydantic import BaseModel
...
def retrieve_activity_logs(models, db, uid, password, lead_id: int) -> List[Dict[str, Any]]:
    try:
        logs = []

        # Fetch lead data for creation and stage changes
        lead_data = models.execute_kw(
            db, uid, password,
            'crm.lead',
            'read',
            [[lead_id]],
            {'fields': ['message_ids', 'stage_id', 'create_date', 'date_last_stage_update']}
        )
        ...
        # Fetch messages using message_ids
        message_ids = lead.get('message_ids', [])
        if message_ids:
            messages = models.execute_kw(
                db, uid, password,
                'mail.message',
                'search_read',
                [[['res_id', '=', lead_id], ['model', '=', 'crm.lead'], ['id', 'in', message_ids]]],
                {'fields': ['date', 'author_id', 'message_type', 'body']}
            )
            for message in messages:
                utc_date = message.get('date', '')
                ist_date = ''
                if utc_date:
                    try:
                        utc_datetime = datetime.strptime(utc_date, '%Y-%m-%d %H:%M:%S')
                        ist_datetime = utc_datetime + timedelta(hours=5, minutes=30)
                        ist_date = ist_datetime.strftime('%Y-%m-%d %H:%M:%S')
                    except ValueError:
                        ist_date = utc_date  # Fallback if parsing fails
                logs.append({
                    "type": "message",
                    "date": utc_date,
                    "ist-time": ist_date,
                    "author": message.get('author_id', [False, "Unknown"])[1] if message.get('author_id') else "System",
        ...
        if create_date:
            try:
                create_utc = datetime.strptime(create_date, '%Y-%m-%d %H:%M:%S')
                create_ist = create_utc + timedelta(hours=5, minutes=30)
                ist_create_date = create_ist.strftime('%Y-%m-%d %H:%M:%S')
            except ValueError:
                ist_create_date = create_date
            logs.append({
                "type": "creation",
                "date": create_date,
                "ist-time": ist_create_date,
                "author": "System",
                "message": ""
            })
        ...
        current_stage = lead.get('stage_id', [False, "Unknown"])[1] if lead.get('stage_id') else "Unknown"
        date_last_stage_update = lead.get('date_last_stage_update', create_date)
        ist_stage_date = ''
        if date_last_stage_update:
            try:
                stage_utc = datetime.strptime(date_last_stage_update, '%Y-%m-%d %H:%M:%S')
                stage_ist = stage_utc + timedelta(hours=5, minutes=30)
                ist_stage_date = stage_ist.strftime('%Y-%m-%d %H:%M:%S')
            except ValueError:
                ist_stage_date = date_last_stage_update
        ...
        if stage_names:
            logs.append({
                ...
                "ist-time": ist_create_date,
                "previous_stage": "None",
                "new_stage": stage_names[0],
                "author": "System",
                "message": ""
            })
            if create_date != date_last_stage_update:
                logs.append({
                    "type": "stage_change",
                    "date": date_last_stage_update,
                    "ist-time": ist_stage_date,
                    "previous_stage": stage_names[0],
        ...
        logger.error(f"Error retrieving activity logs for lead {lead_id}: {str(e)}")
        return []
...
def search_leads(state: AgentState) -> Command[Literal["summarizer", "__end__"]]:
    try:
        ...
        # Search leads using Odoo API (search_read) without specifying fields
        models = odoo_auth['models']
        leads = models.execute_kw(
            odoo_auth['db'], odoo_auth['uid'], odoo_auth['password'],
            'crm.lead', 'search_read',
        ...
        # Fetch activity logs for each lead
        for lead in leads:
            lead_id = lead['id']
            activity_logs = retrieve_activity_logs(models, odoo_auth['db'], odoo_auth['uid'], odoo_auth['password'], lead_id)
            lead['activity_logs'] = activity_logs
...
def summarize_results(state: AgentState) -> Command[Literal["__end__"]]:
    try:
        if state['source'] == "odoo":
            prompt = f"""
            Summarize the following Odoo lead search results for the query '{state['query']}' in a concise manner (max 200 words).
            Focus on key information relevant to the query, such as lead names, partners, probabilities, email, phone, salesperson, expected revenue, stages, and recent activity logs (like stage changes, opportunity creation) if available.
